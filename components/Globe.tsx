...
export function Globe({
  ...
  const updateMovement = (clientX: number, clientY: number) => {
    if (pointerInteracting.current !== null) {
      ...
      const deltaY = clientY - pointerInteracting.current

      // Adjust rotation speed and direction using refs instead of state
      rRef.current = deltaX / 100
      thetaRef.current += deltaY / 100
    }
  }
  ...
  return (
    <div
      className={cn(
        "absolute inset-0 mx-auto aspect-[1/1] w-full max-w-[900px]", // Increased max-width
        className
      )}
    >
      <canvas
